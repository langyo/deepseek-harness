...
**菜单的存在是为了在多个目标之间消歧。** 最初发布时，空且已就绪的列表上的选择器打开请求会通过一个覆盖两处表层的谓词（`addIsTheOnlyEntry`）把锚点手势直接消费进流程。该条款已被[打开纪律笔记](../bug-fix/2026-08-28-no-workspace-empty-state-never-auto-opens.zh.md)取代：检测到空列表永远不打开流程——选择手势渲染带有单独添加一行的菜单——只有明确的添加意图手势（确认无工作区空状态的按钮、仅添加的侧边栏区头）才直接拉起流程。

由该规则派生出两条边界，它们同属这个决定：

- **列表为空只有在基线落地后才算最终结果。** `phase` 仍为 `pending` 时，主视觉区保留菜单与加载状态，而不是把列表当作最终结果；就绪后空列表的处理现在由打开纪律笔记的空状态承载。仅添加表层不列任何东西，因此从不等待。
...
直接拉起的这条路径同样遵守菜单项声明的 busy 规则：某次选取仍在接纳中（`flowBusy`）时，添加意图手势会被拦截，效果与禁用该菜单项完全相同，从而避免迟到的结果与第二个流程发生竞态。

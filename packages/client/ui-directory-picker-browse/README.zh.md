...
对话框的交互中状态会持久化（两个流程界面共用一个草稿键，打开期间经 `readDialogDraft`／`writeDialogDraft` 逐变更写盘）：每次打开重新加载上次的层级，连同路径编辑器的实时文本、隐藏开关、以及打开中的新建文件夹表单草稿——表单填到一半时页面刷新或浏览器崩溃，重开时正好接续原处（配合 ui-workspace 持久化的流程打开请求）。确认选取会清除草稿；被关闭的对话框保留其位置，供下一次主动打开时接续。

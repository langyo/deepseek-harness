...
共享 Workspace 浏览器与选择器插件。`WorkspaceBrowser` 填充侧边栏的 `sidebar.workspaces` slot，`WorkspacePicker` 则填充页面局部 Session Intent 主视觉区的 `conversation.hero.workspace` slot，`WorkspaceHeroEmpty` 填充主视觉区的 `conversation.hero.workspace.empty` slot（确认无工作区的标记与添加操作）；所有界面使用同一套 Workspace 菜单和添加流程。
